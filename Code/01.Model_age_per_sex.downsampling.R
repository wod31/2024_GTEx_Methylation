...
if (tissue == 'BreastMammaryTissue') {
  ...
  metadata$SEX[metadata$sex == 1] <- 0
  metadata$SEX[metadata$sex == 2] <- 1
}

library(MatchIt)
samples_female <-  matchit(SEX ~ AGE, data = metadata, method = "nearest")
index_1 <- (rownames(samples_female$match.matrix))
index_0 <- (c(samples_female$match.matrix[,1]))
metadata <- metadata[c(index_0,index_1),]
...
metadata_male$AGE <- as.numeric(metadata_male$AGE)
...
saveRDS(res_2, paste0(project_path, "/Tissues/", tissue, "/DML_results_5_PEERs_continous.peer.AgeMale.Downsample.rds")) 
print("Using 5 PEERs:")
...
saveRDS(res_2, paste0(project_path, "/Tissues/", tissue, "/DML_results_5_PEERs_continous.peer.AgeFemale.Downsample.rds")) 
print("Using 5 PEERs:")
