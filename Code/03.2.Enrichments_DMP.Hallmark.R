...
# @Description: Get funtional enrichments of DMPs for Hallmark pathways
...
saveRDS(hallmark_enrichments, 'Hallmarks_DMP_peers5.hypo.continous.rds')
...
saveRDS(hallmark_enrichments, 'Hallmarks_DMP_peers5.hyper.continous.rds')
...
for (tissue in tissues) {
  ...
  for (trait in names(results_DML[[tissue]])[!is.na(results_DML[[tissue]])]) {
    ...
    tryCatch(
      {res <- gsameth(rownames(results_DML[[tissue]][[trait]][results_DML[[tissue]][[trait]]$adj.P.Val<0.05,]), all.cpg=(betas),
                      collection=hallmark,array.type="EPIC")
      print(table(res$FDR<0.05))
      if (sum(res$FDR<0.05) > 0) {
        hallmark_enrichments[[tissue]][[trait]] <- res[res$FDR<0.05,]}
...
saveRDS(hallmark_enrichments, 'Hallmarks_DMP_peers5.continous.rds')
