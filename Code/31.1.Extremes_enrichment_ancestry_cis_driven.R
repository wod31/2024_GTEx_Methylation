...
my_fisher <- function(type, tissue, trait){
  ...
  print(type)
  print(tissue)
  cis_driven <- unique(res_all$deg[res_all$Class=='Cis-driven' & res_all$tissue==tissue])
  res <- results_DML[[tissue]][[trait]]
  #res <- res[rownames(res) %in% cis_driven,]
  chrom_tissue <- extremes[[type]][[tissue]]
  
  type_diff <- sum(rownames(res[res$adj.P.Val<0.05 & res$logFC<0 & rownames(res) %in% cis_driven,]) %in% chrom_tissue)
  type_notdiff <- length(chrom_tissue) - type_diff
  other_type_diff <- (nrow(res[res$adj.P.Val<0.05 & res$logFC<0 & rownames(res) %in% cis_driven,]))-type_diff
...
saveRDS(fisher_results, '~/marenostrum/Projects//GTEx_v8/Methylation/Data/enrichment_extremes_AA.eurv1.cis_driven.rds')
...
hyper <- readRDS('~/marenostrum/Projects/GTEx_v8/Methylation/Data/enrichment_extremes_EA.eurv1.cis_driven.rds')
hypo <- readRDS('~/marenostrum/Projects/GTEx_v8/Methylation/Data/enrichment_extremes_AA.eurv1.cis_driven.rds')
...
for (type in c('hyper','hypo')) {
  for (trait in c('EURv1')) {
    ...
    pdf(file = paste0("~/marenostrum/Projects/GTEx_v8/Methylation/Plots/enrichment", gsub('\\/','_',type),'_',trait,".cis_driven.v2.pdf"), w = 8, h = 4)
    print(p)
    dev.off()
  }
}
