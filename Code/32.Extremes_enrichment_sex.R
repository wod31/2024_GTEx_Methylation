...
fisher_results <- lapply(tissues, function(tis) lapply(c('SEX2'), function(trait) lapply(families, function(region) my_fisher(region, tis,trait))))
names(fisher_results) <- tissues

for (name in tissues) {
  names(fisher_results[[name]]) <- 'SEX2'
}
...
saveRDS(fisher_results, '~/marenostrum/Projects//GTEx_v8/Methylation/Data/enrichment_extremes_Male.sex.rds')
...
hyper <- readRDS('~/marenostrum/Projects/GTEx_v8/Methylation/Data/enrichment_extremes_Female.sex.rds')
hypo <- readRDS('~/marenostrum/Projects/GTEx_v8/Methylation/Data/enrichment_extremes_Male.sex.rds')
...
for (type in c('hyper','hypo')) {
  for (trait in c('SEX2')) {
    ...
    hyper_hypo$type[hyper_hypo$type =="hypo"] <- "Male"
    hyper_hypo$type[hyper_hypo$type =="hyper"] <- "Female"
