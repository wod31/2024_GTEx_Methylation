...
# @Description: Plot correlations, explore different threseholds of DVPs and other things
...
get_corr <- function(tissue, trait){
  if(tissue %in% sex_tissues & trait == "Sex"){
    ...
    model <- readRDS(paste0(project_path, "Tissues/",tissue, "/",trait,'_Correlations_probes_genes_DVG_DVP.pnominal.rds'))
    #rownames(model[[trait]][model[[trait]]$adj.P.Val<0.05,])
    model
  }
}
DMPs_Res <- lapply(c("Ancestry", "Sex", "Age"), function(trait) lapply(tissues, function(tissue) get_corr(tissue, trait)))
names(DMPs_Res) <- c("Ancestry", "Sex", "Age")
for(trait in c("Ancestry", "Sex", "Age")){names(DMPs_Res[[trait]]) <- tissues}
...
signif <- lapply(c("Ancestry", "Sex", "Age"), function(trait) lapply(tissues, function(tissue) get_signif(tissue, trait)))
names(signif) <- c("Ancestry", "Sex", "Age")
for(trait in c("Ancestry", "Sex", "Age")){names(signif[[trait]]) <- tissues}
...
DMPs_DEGs <- lapply(c("Ancestry", "Sex", "Age"), function(trait) lapply(tissues, function(tissue) get_pairs(tissue, trait)))
names(DMPs_DEGs) <- c("Ancestry", "Sex", "Age")
for(trait in c("Ancestry", "Sex", "Age")){names(DMPs_DEGs[[trait]]) <- tissues}
...
final_table <- merge(final_table, gene_annotation, all.x=T)
...
get_pairs <- function(tissue, trait){
  if(tissue %in% sex_tissues & trait == "Sex"){
    ...
    length(final_table$symbol[final_table$gene_name %in% meth_genes$UCSC_RefGene_Name & final_table$adj.P.Val<0.05 & final_table$tissue==tissue & final_table$trait==trait])
  }
}

genes_DE_with_probe <- lapply(c("Ancestry", "Sex", "Age"), function(trait) lapply(tissues, function(tissue) get_pairs(tissue, trait)))
names(genes_DE_with_probe) <- c("Ancestry", "Sex", "Age")
for(trait in c("Ancestry", "Sex", "Age")){names(genes_DE_with_probe[[trait]]) <- tissues}

n_samples <- sapply(tissues, function(tissue) nrow(metadata[[tissue]]))
names(n_samples) <- tissues

counts <- sapply(c("Ancestry", "Sex", "Age"), function(trait)
...
counts <- sapply(c("Ancestry", "Sex", "Age"), function(trait)
  sapply(tissues, function(tissue) 
    if (tissue %in% sex_tissues & trait == "Sex") {
      ...
      length(unique(signif[[trait]][[tissue]]$gene))/length(unique(DMPs_DEGs[[trait]][[tissue]]$gene))*100
    }
  ))
...
colnames(counts_num) <- c('Ancestry','Sex','Age')
...
pdf("~/marenostrum/Projects/GTEx_v8/Methylation/Plots/Perc_explained_DVGs_heatmap.pnominal.pdf",
    width = 6, height = 4)
draw(ht)
...
pdf("~/marenostrum/Projects/GTEx_v8/Methylation/Plots/Number_explained_DVGs_heatmap.pnominal.pdf",
    width = 6, height = 4)
draw(ht)
# heatmap_legend_side = "bottom")
dev.off()
